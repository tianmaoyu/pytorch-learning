...
class YoloV5Loss():
    ...
    def __call__(self, predictions: Tensor, targets: Tensor):
        ...
        bs, channel, height, width = predictions.shape
        # 变形
    ...
    def build_targets(self, predictions: Tensor, targets: Tensor):
        """
        :param predictions: (bs,3,h,w,class_num+5) :(1,3,80,80,85)
        :param targets: (image,class,x,y,w,h) : [image,class,x,y,w,h]
        :return:
        """
        anchor_num=3
        # 图片上 目标个数 假设有两个
        ...
        off_set_bias=0.5
        # 上，下，左，右 移动
        offset = torch.tensor([[0, 0], [1, 0], [0, 1], [-1, 0], [0, -1]], device=targets.device).float() * off_set_bias  # 偏移值
